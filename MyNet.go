...
func (conn *MyConn) ReadFromUDP(b []byte)(int,*net.UDPAddr,error){
	...
	raddr := csockToUdpaddr(addr)//return UDPAddr
...
func udpaddrToCsock(addr *net.UDPAddr) (address C.struct_sockaddr_in){
	...
	Cip := C.CString(ip)//allocate memory and copy ip !!(not buf is ok)
	defer C.free(unsafe.Pointer(Cip))
	port := addr.Port
	address.sin_family = syscall.AF_INET
	address.sin_addr.s_addr = C.inet_addr(Cip)//need to free???yes-> free Cip
...
func csockToUdpaddr(address C.struct_sockaddr_in) (addr *net.UDPAddr){
	var ip_buf string
	ip_bufptr := C.CString(ip_buf)
	defer C.free(unsafe.Pointer(ip_bufptr))
	r_port := myHtons(int(C.int(address.sin_port)))//need myHtons to get correct remote port
	C.inet_ntop(syscall.AF_INET,unsafe.Pointer(&address.sin_addr),ip_bufptr,INET_ADDRSTRLEN)
	r_ip := net.ParseIP(C.GoString(ip_bufptr))
	addr = &net.UDPAddr{
		IP: r_ip,
		Port:int(r_port),
	}
